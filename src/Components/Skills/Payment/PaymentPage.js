...
function PaymentSuccessPage() {
 ...
 const { razorpay_payment_id, razorpay_payment_link_id, razorpay_payment_link_reference_id, razorpay_payment_link_status, razorpay_signature } =
   router.query;
...
useEffect(() => {
  ...
  const fetchData = async () => {
   ...
   const response = await fetch(
     `https://upsc.joshtalks.org/api/v1/payment/success/`,
     {
       method: "POST",
       headers: {
      ...
      {error == false && data && !loading ? (
        <div className={styles["childPaymentDiv"]}>
          ...
          <div className={styles["transactionStyle"]}>
           ...
           {data.success_at && <div className={styles["transactionDiv"]}>
              <p
                className={styles["transactionDivP"]}
                style={{
                  fontFamily: "Open Sans",
